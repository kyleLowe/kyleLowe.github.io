...
function App() {
  ...
  useEffect(() => {
    ...
    if (activeModal) {
      ...
      gsap.fromTo(activeModal, { opacity: 0 }, { opacity: 1, duration: 0.5 });
    }
  ...
  useEffect(() => {
    ...
    function handleRaycastingInteraction() {
      ...
      if (currentIntersects.length > 0) {
        ...
        if (
          object.name &&
          object.parent?.name.includes("HomeButton_First_Raycaster")
        ) {
          setModalDisplay("home");
        }
        if (
          object.name &&
          object.parent?.name.includes("ProjectButton_First_Raycaster")
        ) {
      ...
      (glb) => {
        glb.scene.traverse((child) => {
          if ((child as THREE.Mesh).isMesh) {
            ...
            (
              Object.keys(loadedTextures) as Array<keyof typeof loadedTextures>
            ).forEach((key) => {
